...
const PlaceDescreption = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/Placedesc.json");
        const placesArray = response.data.Placedesc;
        console.log(placesArray)
        if (Array.isArray(placesArray)) {
          // Filter the places based on the id
          const selectedPlace = placesArray.find(place => place.id === parseInt(id, 10) && place.name === decodeURIComponent(name));


          // Set the selected place to productData
          if (selectedPlace) {
            setProductData([selectedPlace]);
          }
    ...
    <>
      <div className={classes.main}>
        <div className="flex justify-around">
          ...
          >
            <IoLocation />
            {productData[0]?.name} -- famous tourist spot
          </h1>
        </div>
          ...
          <div className="leftcontent" style={{ width: "50vw" }}>
            <div>
              <h1 className="flex gap-2 font-[500]">
                ...
                <p>{productData[0]?.name} is a famous spot in India</p>
              </h1>
